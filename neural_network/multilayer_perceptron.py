...
class BaseMultilayerPerceptron(six.with_metaclass(ABCMeta, BaseEstimator)):
    ...
    def _resample_with_replacement(self, X, y, sample_weight):
        """makes a new X and y resampled by weights (probabilities)"""
        # normalize sample_weights if not already
        sample_weight = sample_weight / sample_weight.sum(dtype=np.float64)

        X_resampled = np.zeros((len(X), len(X[0])), dtype=np.float32)
        y_resampled = np.zeros((len(y)), dtype=np.int)
        ...
        for i in range(len(X)):
            ...
            X_resampled[i] = X[draw]
            y_resampled[i] = y[draw]
    ...
    def fit(self, X, y, sample_weight=[]):
        ...
        if len(sample_weight) > 0:
            if len(sample_weight) != len(X):
                raise Exception('sample weights do not match X')

            X, y = self._resample_with_replacement(X, y, sample_weight)
